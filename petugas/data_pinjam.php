...
if (
  ...
  if (isset($_POST['kembaliBuku'])) {
    if (kembaliBuku($_POST) > 0) {
      echo "
        <script>
  ...
  if (isset($_POST['bayarDenda'])) {
    if (bayarDenda($_POST) > 0) {
      echo "
        <script>
        alert('Berhasil membayar denda');
        document.location.href = 'data_pinjam.php';
        </script>
       ";
...
<div class="container">
    ...
    <h2 class="text-bold text-center">Halaman data peminjaman</h2>
<table class="table table-light border-primary text-center">
  ...
  if($search){
    ...
    INNER JOIN user ON peminjaman.id_user = user.id_user WHERE judul LIKE ? OR username LIKE ? OR tanggal_pinjam LIKE ? OR tanggal_kembali LIKE ?");
    $search_param = '%' . $search . '%';
...
<form action="" method="post">
  <tbody>
    ...
    <input type="hidden" name="tanggal_deadline" value="<?= htmlspecialchars($data['tanggal_deadline']) ?>">
    ...
    <input type="hidden" name="id_pinjam" value="<?= htmlspecialchars($data['id_pinjam']) ?>">
    <input type="hidden" name="id_buku" value="<?= htmlspecialchars($data['id_buku']) ?>">
    <button class="btn btn-danger" name="kembaliBuku" type="submit">Konfirmasi pengembalian buku</button>

          <?php } elseif ($data['status_peminjaman'] === 'Sudah Dikembalikan'){?>
     <button class="btn btn-success" disabled>Buku Sudah diterima</button>
          <?php } ?>
     </td>

     <td>
      <?php if ($data['denda'] == 0) { ?>
        <button class="btn btn-success" disabled>Tidak ada denda</button>
     
       <?php } else { if ($data['status_bayar'] === 'Belum Dibayar'){?>
    <input type="hidden" name="id_pinjam" value="<?= htmlspecialchars($data['id_pinjam']) ?>">
    <button class="btn btn-danger" name="bayarDenda" type="submit">Konfirmasi pembayaran denda</button>

          <?php } elseif ($data['status_bayar'] === 'Sudah Dibayar'){?>
     <button class="btn btn-success" disabled>Denda Sudah dibayar</button>
          <?php }
          } ?>
    </td>
    </tr>
  ...
  <?php 
            ...
            <tr><td colspan="10" class="empty-book">Belum ada peminjaman buku</td></tr>;
       <?php } ?>
</table>
    </div>
</div>
