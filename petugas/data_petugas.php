...
if (
  ...
  if (isset($_POST['aktifPetugas'])) {
    if (aktifPetugas($_POST) > 0) {
      echo "
        <script>
        alert('Berhasil mengaktifkan akun');
        document.location.href = 'data_akun_petugas.php';
        </script>
       ";
  ...
  if (isset($_POST['nonaktifPetugas'])) {
    if (nonaktifPetugas($_POST) > 0) {
      echo "
        <script>
        alert('Berhasil menonaktifkan akun');
        document.location.href = 'data_akun_petugas.php';
        </script>
       ";
  ...
  if (isset($_POST['deletePetugas'])) {
    if (deletePetugas($_POST) > 0) {
      echo "
        <script>
        alert('Berhasil menghapus akun petugas');
...
<div class="container">
    ...
    <h2 class="text-bold text-center">Halaman data akun librarian</h2>
<table class="table table-light border-primary text-center">
  ...
  <?php 
   ...
   if($search){
     $stmt = $conn->prepare("SELECT * FROM petugas WHERE username LIKE ? OR email LIKE ? OR id_petugas LIKE ? OR no_hp LIKE ?");
     $search_param = '%' . $search . '%';
     $stmt->bind_param("ssis", $search_param, $search_param, $search_param, $search_param);
     $stmt->execute();
   } else {
  $stmt = $conn->prepare("SELECT * FROM petugas ORDER BY id_petugas ASC");
  $stmt->execute();
}
...
<form action="" method="post">
  <tbody>
    <tr class="mt-2">
    <td><?= $angka++ ?></td>
    <td><?= htmlspecialchars($data['id_petugas'])  ?? '' ?></td>
    ...
    <a href="edit_petugas.php?id_petugas=<?=  htmlspecialchars($data['id_petugas'])  ?>" class="btn btn-primary" >Edit</a>
      <a class="btn btn-danger" onclick="hapusPetugas('delete_petugas.php?id_petugas=<?=htmlspecialchars($data['id_petugas'])?>')">Hapus</a>
    

    <?php if ($data['status'] === 'Tidak Aktif'){ ?>
      <form action="" method="post">
          <input type="hidden" name="id_petugas" value="<?= htmlspecialchars($data['id_petugas']) ?>">
          <button class="btn btn-success" name="aktifPetugas" type="submit">Aktifkan</button>
          </form>

      <?php } elseif ($data['status'] === 'Aktif'){ ?>
        <form action="" method="post"> 
          <input type="hidden" name="id_petugas" value="<?= htmlspecialchars($data['id_petugas']) ?>">
          <button class="btn btn-success" name="nonaktifPetugas" type="submit">Nonaktifkan</button>
          </form>
    </td>
...
<?php } ?>
  ...
  <?php 
        ...
        <tr><td colspan="7" class="empty-book">Belum ada akun petugas</td></tr>;
   <?php }?>
</table>
    </div>
</div>
